...
// When the archive outgrows the logo's cells, a few cells along the path are
// split one size finer, just enough to fit — spread by a golden-ratio stride
// so no one stretch of the spiral turns fine all at once.
function fitCells(base, need) {
    const k = base.slice();
    let cap = k.reduce((a, v) => a + v * v, 0);
    for (let j = 1; cap < need && j < 100000; j++) {
        const i = Math.floor(((j * 0.6180339887) % 1) * k.length);
        if (k[i] >= 6) continue;
        cap += (k[i] + 1) ** 2 - k[i] ** 2;
        k[i]++;
    }
...
(async function () {
    ...
    // Interviews come first in the data (other scripts point at them by
    // index), but every piece is laid along the path by publication date.
    const order = rows.map((r, i) => i).sort((a, b) =>
        rows[a][2] < rows[b][2] ? -1 : rows[a][2] > rows[b][2] ? 1 : a - b);
    const CELLS = fitCells(SPIRAL_K, rows.length);
    ...
    svg.setAttribute('aria-label', `${rows.length} pieces from the archive, oldest at the centre of the spiral`);
    ...
    // rects are in path order, so each one looks up its own piece
    const days = rects.map(r => dayOf(rows[+r.dataset.i][2]));
    ...
    function play() {
        ...
        const step = (now) => {
            ...
            while (shownTo < rects.length && days[shownTo] <= day) {
                const year = rows[+rects[shownTo].dataset.i][2].slice(0, 4);
                if (year !== lastYear) {
                    lastYear = year;
